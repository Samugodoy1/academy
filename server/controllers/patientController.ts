...
const getProduct = (req: Request) => req.user?.product || 'academy';
...
export const getPatientById = async (req: Request, res: Response) => {
  ...
  const { id } = req.params;
  try {
    const patientResult = await query('SELECT * FROM patients WHERE id = $1 AND dentist_id = $2 AND product = $3', [id, user.id, product]);
    ...
    const [anamnesis, evolution, files, odontogram, toothHistory, consents] = await Promise.all([
      ...
      query('SELECT * FROM clinical_evolution WHERE patient_id = $1 AND product = $2 ORDER BY date DESC', [id, product]),
      query('SELECT * FROM patient_files WHERE patient_id = $1 AND product = $2 ORDER BY created_at DESC', [id, product]),
      ...
      query(`
        ...
        WHERE th.patient_id = $1 AND th.product = $2
...
export const updatePatient = async (req: Request, res: Response) => {
  ...
  try {
    const checkOwnership = await query('SELECT id FROM patients WHERE id = $1 AND dentist_id = $2 AND product = $3', [id, user.id, product]);
    ...
    await query(
      `UPDATE patients 
       ...
       WHERE id = $10 AND dentist_id = $11 AND product = $12`,
      [name, cpf, birth_date, phone, email, address, photo_url, JSON.stringify(treatmentPlan || []), JSON.stringify(procedures || []), id, user.id, product]
    );
...
export const addEvolution = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await query(
      'INSERT INTO clinical_evolution (patient_id, notes, procedure_performed, materials, observations, dentist_id, product) VALUES ($1, $2, $3, $4, $5, $6, $7)',
      [id, notes, procedure_performed, materials, observations, user.id, product]
    );
...
export const addToothHistory = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await query(
      'INSERT INTO tooth_history (patient_id, dentist_id, product, tooth_number, procedure, notes, date) VALUES ($1, $2, $3, $4, $5, $6, $7)',
      [id, user.id, product, tooth_number, procedure, notes, date || new Date().toISOString().split('T')[0]]
    );
    return res.status(201).json({ success: true });
  } catch (error: any) {
...
export const deleteToothHistory = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await query(
      'DELETE FROM tooth_history WHERE patient_id = $1 AND tooth_number = $2 AND product = $3',
      [id, toothNumber, product]
    );
    return res.status(200).json({ success: true });
  } catch (error: any) {
...
export const addPatientFile = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await query(
      'INSERT INTO patient_files (patient_id, product, file_url, file_type, description) VALUES ($1, $2, $3, $4, $5)',
      [id, product, file_url, file_type, description]
    );
    return res.status(201).json({ success: true });
  } catch (error: any) {
