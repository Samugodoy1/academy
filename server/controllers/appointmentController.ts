...
const syncAppointmentStatusesForDentist = async (dentistId: number, product: string) => {
  await query(
    `
      UPDATE appointments
      SET status = CASE
...
export const getAppointments = async (req: Request, res: Response) => {
  ...
  try {
    await syncAppointmentStatusesForDentist(user.id, product);
    ...
    const sql = `
      ...
      WHERE a.dentist_id = $1
        ...
        AND p.product = $2
    `;
    const result = await query(sql, [user.id, product]);
...
export const createAppointment = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const patientCheck = await query('SELECT id FROM patients WHERE id = $1 AND dentist_id = $2 AND product = $3', [patient_id, user.id, product]);
    ...
    const conflictCheck = await query(
      ...
      `,
      [user.id, start_time, end_time, product]
    );
    ...
    const result = await query(
      'INSERT INTO appointments (patient_id, dentist_id, product, start_time, end_time, notes) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id',
      [
        patient_id,
        user.id,
        product,
...
export const updateAppointmentStatus = async (req: Request, res: Response) => {
  ...
  try {
    const result = await query('UPDATE appointments SET status = $1 WHERE id = $2 AND dentist_id = $3 AND product = $4', [status, id, user.id, product]);
...
export const updateAppointment = async (req: Request, res: Response) => {
  ...
  try {
    const check = await query('SELECT id FROM appointments WHERE id = $1 AND dentist_id = $2 AND product = $3', [id, user.id, product]);
    ...
    await query(
      'UPDATE appointments SET patient_id = $1, start_time = $2, end_time = $3, notes = $4 WHERE id = $5 AND dentist_id = $6 AND product = $7',
      [patient_id, start_time, end_time, notes, id, user.id, product]
    );
