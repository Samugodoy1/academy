...
export const uploadPatientFile = async (req: Request, res: Response) => {
  const user = req.user!;
  const product = user.product || 'academy';
  const { id } = req.params;
  ...
  try {
    ...
    const result = await query(
      'INSERT INTO patient_files (patient_id, product, file_url, file_public_id, file_type, description) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *',
      [id, product, file.path, file.filename, file_type || 'image', description]
    );
...
export const uploadPatientPhoto = async (req: Request, res: Response) => {
  ...
  try {
    const checkOwnership = await query('SELECT id, photo_public_id FROM patients WHERE id = $1 AND dentist_id = $2 AND product = $3', [id, user.id, product]);
    if (checkOwnership.rows.length === 0) {
      if (file.filename) await deleteImage(file.filename);
      return res.status(403).json({ error: 'Acesso negado' });
    }
    ...
    await query(
      'UPDATE patients SET photo_url = $1, photo_public_id = $2 WHERE id = $3 AND dentist_id = $4 AND product = $5',
      [file.path, file.filename, id, user.id, product]
    );
