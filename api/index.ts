...
const blockAcademyCommercialModules = (req: any, res: any, next: any) => {
  if (req.user?.product === 'academy') {
    return res.status(404).json({ error: 'Modulo indisponivel no Academy' });
  }
  return next();
};

app.use(
  ['/finance', '/api/finance', '/intelligence', '/api/intelligence', '/ml', '/api/ml', '/portal', '/api/portal'],
  blockAcademyCommercialModules
);
